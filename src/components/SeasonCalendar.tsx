...
export function SeasonCalendar({ currentRound, daysUntilRace }: Props) {
  return <section className="season-calendar">
    <header><div><p className="panel-label">2026 FORMULA 1 CALENDAR</p><h2>RACE SCHEDULE</h2></div><div className="date-card"><b>{daysUntilRace === 0 ? 'RACE WEEKEND' : `${daysUntilRace} DAYS`}</b><small>{daysUntilRace === 0 ? 'READY TO RACE' : 'SKIP DAYS IN TEAM & GARAGE'}</small></div></header>
