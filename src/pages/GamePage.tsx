...
import { CareerTutorial } from '../components/CareerTutorial';
import { CareerExtras } from '../components/CareerExtras';
import { CareerSlots } from '../components/CareerSlots';
import { DeviceChooser } from '../components/DeviceChooser';
import { DailyStreak } from '../components/DailyStreak';
import { HqCategoryTabs, type HqCategory } from '../components/HqCategoryTabs';
...
import { createCareerLoan, loadCareerLoan, updateCareerLoan, type CareerLoan } from '../lib/loans';
...
export function GamePage() {
  ...
  const [loan, setLoan] = useState<CareerLoan | null>(null);
  const [hqCategory, setHqCategory] = useState<HqCategory>('team');
  ...
  const refundUpgrade = () => { if (upgrades.length === 0) return; const refund = Math.round(8 * 1.5 ** (upgrades.length - 1)); setUpgrades((value) => value.slice(0, -1)); setPace((value) => Math.max(team.pace, value - 2)); setBudget((value) => value + refund); };
  const refundPitCrew = () => { if (pitCrewLevel === 1) return; const refund = Math.round(6 * 1.5 ** (pitCrewLevel - 2)); setPitCrewLevel((value) => value - 1); setBudget((value) => value + refund); };
  ...
  const finishRace = (prize: number, playerPosition: number, teammatePosition: number) => { const points = [25, 18, 15, 12, 10, 8, 6, 4, 2, 1]; const teammate = team.driver === driver ? team.teammate : team.driver; const track = calendar[round - 1] ?? 'Season finale'; const totalIncome = prize + sponsorBonus(sponsor); setBudget((value) => value + totalIncome); setDriverRating((value) => Math.min(100, value + .625)); setRacesCompleted((value) => Math.min(24, value + 1)); setDriverPoints((value) => ({ ...value, [driver]: (value[driver] ?? 0) + (points[playerPosition - 1] ?? 0), [teammate]: (value[teammate] ?? 0) + (points[teammatePosition - 1] ?? 0) })); setRaceHistory((value) => [...value, { round, track, position: playerPosition, teammatePosition, prize: totalIncome }]); setRound((value) => Math.min(24, value + 1)); setDaysUntilRace(14); if (loan) { const remainingRaces = Math.max(0, loan.remainingRaces - 1); const nextLoan = { ...loan, remainingRaces }; setLoan(nextLoan); void updateCareerLoan(loan.id, { remainingRaces }); } if (round === 24) setContractYears((value) => Math.max(0, value - 1)); };
  const applyForLoan = (amount: number, reason: string, term: number) => { if (!userId || slot === null) return; void createCareerLoan(userId, slot, amount, reason, term).then((nextLoan) => { if (!nextLoan) return; setLoan(nextLoan); setBudget((value) => value + amount); }); };
  const repayLoan = () => { if (!loan || budget < loan.repayment) return; setBudget((value) => value - loan.repayment); void updateCareerLoan(loan.id, { status: 'repaid' }); setLoan(null); };
  const openWeekend = () => { if (loan?.remainingRaces === 0) { void updateCareerLoan(loan.id, { status: 'defaulted' }); setLoan(null); setUpgrades([]); setPace(team.pace); window.alert('Loan defaulted: the bank repossessed all car upgrades.'); return; } localStorage.setItem('apex-active-upgrade-count', String(upgrades.length)); setView('weekend'); };
  const start = (years: number, name: string | null) => { if (name) { setTeam((value) => ({ ...value, driver: name })); setDriver(name); } setContractYears(years); setHqCategory('team'); setTutorialOpen(true); setView('hq'); };
  ...
  const careerControls = <CareerControls sponsor={sponsor} contractYears={contractYears} budget={budget} onSponsor={setSponsor} onRenew={() => { const cost = 12 + (3 - Math.min(contractYears, 3)) * 4; setBudget((value) => value - cost); setContractYears((value) => Math.min(5, value + 1)); }} onChangeTeam={() => setView('pick')} />;
  return <main className="game-shell"><nav className="topbar"><Link href="/" className="brand">APEX <i>RACING</i></Link><span><LanguageSelector /> MANAGER CAREER</span></nav>{view === 'pick' && <TeamPicker teams={teams} selected={team} year={year} onYear={setYear} onSelect={choose} onStart={start} />}{view === 'hq' && <><div className="hq hq-category-bar"><HqCategoryTabs active={hqCategory} onChange={setHqCategory} /></div>{hqCategory === 'team' && <TeamHQ team={team} driver={driver} driverRating={driverRating} racesCompleted={racesCompleted} teamDrivers={teamDrivers} gridDrivers={gridDrivers} raceHistory={raceHistory} driverPoints={driverPoints} budget={budget} pace={pace} upgrades={upgrades} market={driverMarketByYear[year]} round={round} contractYears={contractYears} canSwitch={contractYears === 0} pitCrewLevel={pitCrewLevel} daysUntilRace={daysUntilRace} onUpgrade={upgrade} onPitCrew={upgradePitCrew} onRefundPitCrew={refundPitCrew} onRefundUpgrade={refundUpgrade} onHire={hire} onTrain={train} onDriver={setDriver} onWeekend={openWeekend} onSkipToRace={() => setDaysUntilRace(0)} onSwitch={() => setView('pick')} />}{hqCategory === 'season' && <div className="hq category-content"><SeasonCalendar currentRound={round} daysUntilRace={daysUntilRace} />{careerControls}<DailyStreak /></div>}{hqCategory === 'finance' && <div className="hq category-content"><BankCenter loan={loan} budget={budget} onApply={applyForLoan} onRepay={repayLoan} /></div>}{hqCategory === 'career' && <div className="hq category-content"><CareerExtras driver={driver} driverRating={driverRating} history={raceHistory} round={round} /><AiAssistant /><SettingsPanel /></div>}{tutorialOpen && <CareerTutorial onClose={() => setTutorialOpen(false)} />}</>}{view === 'weekend' && <RaceWeekend pace={pace} driverRating={driverRating} pitCrewLevel={pitCrewLevel} driver={driver} teammate={teammate} gridDrivers={gridDrivers} round={round} track={calendar[round - 1]} onBack={() => setView('hq')} onRaceFinish={finishRace} />}</main>;
}
