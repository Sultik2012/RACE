...
export function GamePage() {
  ...
  useEffect(() => { if (!hydrated || view !== 'hq' || daysUntilRace === 0) return; const timer = window.setInterval(() => setDaysUntilRace((days) => Math.max(0, days - 1)), 60000); return () => window.clearInterval(timer); }, [hydrated, view, daysUntilRace]);
  ...
  const openWeekend = () => { if (daysUntilRace > 0) { window.alert(`The next Grand Prix is in ${daysUntilRace} game day(s). One real minute equals one game day.`); return; } if (loan?.remainingRaces === 0) { void updateCareerLoan(loan.id, { status: 'defaulted' }); setLoan(null); setUpgrades([]); setPace(team.pace); window.alert('Loan defaulted: the bank repossessed all car upgrades.'); return; } setView('weekend'); };
